...
def write_wav(list_file, write_path, outfile_name):
    ...
    for line in infile:
        ...
        file = (
            os.path.splitext(src_path.split("/")[-1])[0]
            + os.path.splitext(src_path.split("/")[-1])[1].lower()
        )  # .WAV -> .wav
...
for i in range(1, 9):
    ...
    test_speaker_folders += [
        entry.path
        for entry in os.scandir(os.path.join(TEST_path, f"DR{i}"))
        if entry.name in test_spklist
    ]
    ...
    for s in speakers[0:6]:
        train_list += [
            entry.path
            for entry in os.scandir(os.path.join(r, s))
            if (entry.name.endswith(".WAV") and not entry.name.startswith("SA"))
        ]  # SA sentances are excluded
...
test_list = [
    entry.path
    for folder in test_speaker_folders
...
write_train_paths = write_wav(
    list_file="CoreTrainList.txt",
    write_path=os.path.join(target_root, "train/clean/"),
    outfile_name="target_CoreTrainList.txt",
)
write_test_paths = write_wav(
    list_file="CoreTestList.txt",
    write_path=os.path.join(target_root, "test/clean/"),
    outfile_name="target_CoreTestList.txt",
)
