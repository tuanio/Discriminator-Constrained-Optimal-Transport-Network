...
if __name__ == "__main__":
    target_root = (
        "/mnt/Datasets/Corpus/VCTK_DEMAND_TBUS_TCAR_TMETRO_to_SCAFE_STRAFFIC_SPSQUARE"
    )
    ...
    for stage in ["train", "test"]:
        ...
        for wav_file in tqdm(wav_files):
            ...
            out_path = wav_file.replace(train_path, train_convert_save_path).split(
                ".w"
            )[0]
            ...
            for i in np.arange(data.shape[1] // n_frame):
                ...
                torch.save(
                    torch.from_numpy(data.transpose()[i * n_frame : (i + 1) * n_frame]),
                    out_name,
                )
