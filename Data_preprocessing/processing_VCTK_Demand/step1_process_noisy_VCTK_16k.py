...
def generate_noisy(noise, clean, SNR, output_path, noise_name, clean_name):
    output_file = os.path.join(
        os.path.join(output_path, noise_name, str(SNR) + "dB"), clean_name
    )
...
for mode in ["train", "test"]:
    for s in ["clean", "noisy"]:
        path = os.path.join(
            save_folder, f"VCTK_DEMAND_{source_domain}_to_{target_domain}/{mode}/{s}/"
        )
...
train_clean_files, dat_clean_files = train_test_split(
    [file.name for file in os.scandir(VCTK_train_path) if file.name.endswith(".wav")],
    test_size=0.3,
)

male_utterances = [
    [file.name for file in os.scandir(VCTK_train_path) if file.name.startswith(spk)]
    for spk in M
]
...
train_clean_files = [file for spk in train_list_M for file in spk] + [
    file for spk in train_list_F for file in spk
]
test_clean_files = [
    file.name for file in os.scandir(VCTK_test_path) if file.name.endswith(".wav")
]


for file in train_clean_files:
    resampling(
        input=os.path.join(VCTK_train_path, file),
...
for file in test_clean_files:
    resampling(
        input=os.path.join(VCTK_test_path, file),
